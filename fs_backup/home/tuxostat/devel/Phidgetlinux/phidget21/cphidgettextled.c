...
CCREATE(TextLED, PHIDDEF_TEXTLED)

CGET(TextLED,RowCount,int)
	TESTPTRS(phid,pVal) 
	TESTDEVICETYPE(PHIDDEF_TEXTLED)
	TESTATTACHED
...
CSETINDEX(TextLED, DisplayString, char *)
	size_t length = strlen(newVal), length_temp = 0;
	int i = 0;
	TESTPTR(phid)
	TESTDEVICETYPE(PHIDDEF_TEXTLED)
	TESTATTACHED
	...
	if (length > phid->phid.attr.textled.numColumns) return EPHIDGET_INVALIDARG;
