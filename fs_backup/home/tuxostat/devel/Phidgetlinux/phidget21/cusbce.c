			...
			if (!found)
			{
				...
				for (i = 0; i<PHIDGET_DEVICE_COUNT; i++) 
				{
						...
						&& (attributes.InterfaceNumber == Phid_Device_Def[i].pdd_iid))
					{
						...
						phid->deviceType = Phid_DeviceName[
						    Phid_Device_Def[i].pdd_did];
	...
	do {
		...
		for (i = 0; i<PHIDGET_DEVICE_COUNT; i++) 
		{
				...
				&& (attributes.InterfaceNumber == Phid_Device_Def[i].pdd_iid))
			{
				...
				if ((Phid_Device_Def_WorkAroundA[i][0]) && (Phid_Device_Def_WorkAroundA[i][1] >= phid->deviceVersion))
				{
					LOG(PHIDGET_LOG_DEBUG,"Sending workaround startup packet");
					ZEROMEM(buffer,8);
