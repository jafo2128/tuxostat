...
FindActiveDevices()
{
	...
	for (trav2=AttachedDevices; trav2; trav2 = trav2->next)
	{
		for (trav=ActiveDevices; trav; trav = trav->next)
		{
			...
			{
				if(!CPhidget_statusFlagIsSet(trav->phid, PHIDGET_ATTACHED_FLAG))
				{
...
CThread_func_return_t ReadThreadFunction(CThread_func_arg_t lpdwParam)
{
	...
	case PHIDDEF_SERVO:
		...
		break;
	case PHIDDEF_INTERFACEKIT:
		if(phid->deviceIDSpec == PHIDSPEC_INTERFACEKIT_0_0_4 && phid->deviceVersion < 704)
			goto exit_not_needed;
		break;
	case PHIDDEF_LED:
	case PHIDDEF_TEXTLED:
	case PHIDDEF_TEXTLCD:
exit_not_needed:
			...
			case EPHIDGET_TIMEOUT:
				...
				case PHIDDEF_RFID:
					if(phid->deviceVersion >= 201)
						...
						goto timeout_ok;
				case PHIDDEF_ENCODER:
					goto timeout_ok;
				default:
					goto timeout_not_ok;
				}
			timeout_not_ok:
				CPhidget_setStatusFlag(phid, PHIDGET_USB_ERROR_FLAG);
...
CThread_func_return_t WriteThreadFunction(CThread_func_arg_t lpdwParam)
{
	...
	{
	case PHIDDEF_INTERFACEKIT:
		if(phid->deviceIDSpec == PHIDSPEC_LINEAR_TOUCH
			|| phid->deviceIDSpec == PHIDSPEC_ROTARY_TOUCH)
			goto exit_not_needed;
		break;
	case PHIDDEF_RFID:
		if(phid->deviceIDSpec == PHIDSPEC_RFID)
			goto exit_not_needed;
		break;
	case PHIDDEF_ENCODER:
	case PHIDDEF_ACCELEROMETER:
	case PHIDDEF_TEMPERATURESENSOR:
	case PHIDDEF_PHSENSOR:
	case PHIDDEF_WEIGHTSENSOR:
exit_not_needed:
...
int CThread_wait_on_event(EVENT *ev, EVENT_TIME time)
{
    ...
    if(ev->ready_to_go == PFALSE)
    {
		if(time == INFINITE)
			...
			timeout.tv_nsec = now.tv_usec*1000 + (time * 1000000);
