...
CPHIDGETINIT(RFID)
	...
	switch(phid->phid.deviceIDSpec)
	{
		case PHIDSPEC_RFID:
			...
			break;
		case PHIDSPEC_RFIDB: //4-output RFID
	...
	while (CPhidget_statusFlagIsSet((CPhidgetHandle)phid, PHIDGET_ATTACHED_FLAG) && phid->tagTimerThread.thread_status == PTRUE)
	{
		if(phid->tagPresent != PFALSE)
		{
			...
			if(duration > 0.2)
			{
				...
				else //could be PUNK_BOOL - don't send event, just set to PFALSE
					phid->tagPresent = PFALSE;
			}
		}
		SLEEP(50);
...
CGET(RFID,AntennaOn,int)
	...
	TESTDEVICETYPE(PHIDDEF_RFID)
	TESTATTACHED
	TESTMASGN(antennaState, PUNK_BOOL)
	MASGN(antennaState)
}
...
CSET(RFID,LEDOn,int)
	...
	if(phid->phid.networkInfo)
		ADDNETWORKKEY(LEDOn, "%d");
	else
	{
		SENDPACKET(RFID, ledState);
		//echo back state if the device doesn't
		if (!(phid->fullStateEcho))
			phid->ledState = newVal;
	}
