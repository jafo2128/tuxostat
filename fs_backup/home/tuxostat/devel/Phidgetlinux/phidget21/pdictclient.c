...
int pdc_get_status(pdc_session_t *pdcs)
{
	return pdcs->status;
}
...
pdc_session_alloc(int readfd, int(*readfunc)(int, void *, unsigned int, char *,
    ...
    int), int (*closefunc)(int, char *, int), void *cleanupPtr, void (*cleanupFunc)(void *), char *password)
{
	pdc_session_t *pdcs;
#ifndef _WINDOWS
	sigset_t new, old;
...
ignore:
	...
	if (ptree_contains(&l, (ptree_node_t *)pdcs->pdcs_listeners, lcmp,
		...
		free(ueval); ueval = NULL;
	}
end:
	free(idstr); idstr = NULL;
	free(key); key = NULL;
}
...
next:
	...
	if(!strncmp(line, "999", 3))
	{
		...
		randomHash = strtol(line + 4, NULL, 0);
		
		LOG(PHIDGET_LOG_DEBUG,"Got hash: %d",randomHash);
		
		if(pdcs->password)
		{
			snprintf(buf,100,"%d%s",randomHash,pdcs->password);
			free(pdcs->password); pdcs->password = NULL;
		}
...
#endif
#endif
		if (!pdcs->pdcs_write(pdcs->pdcs_wfd, (char *)buf,
		    (int)strlen(buf), pdcs->pdcs_resultreader_errdesc,
		    sizeof (pdcs->pdcs_resultreader_errdesc))) {
			LOG(PHIDGET_LOG_DEBUG,"Error in read_results: %s",
			    pdcs->pdcs_resultreader_errdesc);
			pdcs->status = 2;
			return (void *)(-1);
		}
	...
	// Authentication
	if(!strncmp(line, "998", 3))
	{
		LOG(PHIDGET_LOG_DEBUG,"Authentication failed");
		pdcs->status = 2;
		return (void *)(-1);
	}
	
	// Authentication
	if(!strncmp(line, "996", 3))
	{
		pdcs->status = 1;
		goto next;
	}
    ...
    const char *line)
{
	...
	if (final) {
		if (pthread_mutex_lock(&grcba->grcba_lock))
			abort();
		grcba->grcba_code = code;
		if (grcba->grcba_reply_len)
			strncpy(grcba->grcba_reply, line,
			    grcba->grcba_reply_len - 1);
		if (pthread_mutex_unlock(&grcba->grcba_lock))
			abort();
		if (pthread_cond_signal(&grcba->grcba_cv))
			abort();
    ...
    void *), void *arg, char *errdesc, int errlen)
{
	...
	if (!ptree_replace(l, (ptree_node_t **)&pdcs->pdcs_listeners, lcmp,
	    NULL)) {
		free(buf); buf = NULL;
		free(l); l = NULL;
		if (errdesc)
    ...
    void (*error)(const char *errdesc, void *arg), void *arg)
{
	...
	if (!(r = malloc(sizeof (*r))) || !(aca = malloc(sizeof (*aca)))) {
		error(strerror(errno), arg);
		return;
	}
	r->r_tag = tag++;
	r->r_notify = async_cmd_cb;
	r->r_arg = aca;
    ...
    int errlen)
{
	...
	res = jcmd(pdc, buf, 200, reply, sizeof (reply), errdesc, errlen);
	free(buf); buf = NULL;
...
{
	return pthread_join(pdcs->pdcs_resultreader, status);
}
