					...
					while (pdd->pdd_did)
					{
							...
							&& interface_num == pdd->pdd_iid)
						{
							...
							if (Attributes.VersionNumber < 0x100) 
								...
								phid->deviceVersion = ((Attributes.VersionNumber >>
									8) * 100) + ((Attributes.VersionNumber &
									0xff));
	...
	{	
		...
		if (HidD_GetAttributes (phid->deviceHandle, &Attributes))
		{
			if (!(res = CUSBGetDeviceCapabilities(phid, phid->deviceHandle)))
			{
				...
				phid->attr = Phid_Device_Def[phid->deviceIDSpec].pdd_attr;
				...
				if ((Phid_Device_Def_WorkAroundA[i][0]) &&
					(Phid_Device_Def_WorkAroundA[i][1] >= phid->deviceVersion))
				{
					unsigned char buffer[8];
					ZEROMEM(buffer,8);
					LOG(PHIDGET_LOG_INFO,"Sending workaround startup packet");
					if(!(res = CUSBSendPacket((CPhidgetHandle)phid, buffer)))
					{
						LOG(PHIDGET_LOG_ERROR, "CUSBSendPacket returned nonzero code: %d", res);
					}
				}
...
wait_for_write_complete:
	wait_return = WaitForSingleObject ((HANDLE)phid->asyncWrite.hEvent, 500);
	switch(wait_return)
	{
