...
#define TESTATTACHED if (!(phid->phid.status & PHIDGET_ATTACHED_FLAG))	return EPHIDGET_NOTATTACHED;
...
#define FIRE(ename, ...) \
if (phid->fptr##ename) \
	if(CPhidget_statusFlagIsSet((CPhidgetHandle)phid, PHIDGET_ATTACHED_FLAG)) \
...
{ \
	...
	CPhidget_clearStatusFlag(&phid->phid, PHIDGET_ATTACHED_FLAG); \
...
#define ADDNETWORKKEYINDEXED(kname, valType) \
do{ \
	char key[1024], val[1024]; \
	if (!phid->phid.networkInfo->server || !CPhidgetSocketClient_statusFlagIsSet(phid->phid.networkInfo->server, PHIDGETSOCKET_ATTACHED_FLAG)) \
...
#define ADDNETWORKKEY(kname, valType) \
do{ \
	char key[1024], val[1024]; \
	if(!phid->phid.networkInfo->server || !CPhidgetSocketClient_statusFlagIsSet(phid->phid.networkInfo->server, PHIDGETSOCKET_ATTACHED_FLAG)) \
		return EPHIDGET_NETWORK_NOTCONNECTED; \
