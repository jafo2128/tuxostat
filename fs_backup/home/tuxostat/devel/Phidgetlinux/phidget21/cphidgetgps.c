...
CPHIDGETDATA(GPS)
	...
	while ((i < 8) && ((unsigned char)(phid->sckbuf_write + 1) != phid->sckbuf_read))
	{
...
CSET(GPS,PositionChangeTrigger,double)
	...
	if(phid->phid.networkInfo)
		...
		phid->PositionChangeTrigger = newVal * 0.0000899928055396;
	return EPHIDGET_OK;
}
