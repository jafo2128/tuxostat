...
CPHIDGETCLEARVARS(PHSensor)
	...
	phid->phMax = PUNK_DBL;
	phid->phMin = PUNK_DBL;
	phid->potentialMax = PUNK_DBL;
	phid->potentialMin = PUNK_DBL;

	phid->Temperature = PUNK_DBL;
	phid->PH = PUNK_DBL;
	phid->PHLastTrigger = PUNK_DBL;
	phid->Potential = PUNK_DBL;
	phid->PHChangeTrigger = PUNK_DBL;
...
CCREATE(PHSensor, PHIDDEF_PHSENSOR)
...
CGET(PHSensor,PH,double)
	TESTPTRS(phid,pVal) 
	TESTDEVICETYPE(PHIDDEF_PHSENSOR)
	TESTATTACHED
