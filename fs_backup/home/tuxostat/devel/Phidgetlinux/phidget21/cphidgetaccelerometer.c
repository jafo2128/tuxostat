...
CPHIDGETCLEARVARS(Accelerometer)
	...
	for (i = 0; i<ACCEL_MAXAXES; i++)
	{
		...
		phid->axisChangeTrigger[i] = PUNK_DBL;
	}
	return EPHIDGET_OK;
}
	...
	switch(phid->phid.deviceIDSpec)
	{
		...
		case PHIDSPEC_ACCELEROMETER3:
			if ((phid->phid.deviceVersion  >= 400) && (phid->phid.deviceVersion  < 500))
			{
				...
				return EPHIDGET_UNEXPECTED;
			break;
		case PHIDSPEC_ACCELEROMETER_w_GYRO:
			if (phid->phid.deviceVersion  < 200)
			{
				phid->accelerationMax = 3.1;
				phid->accelerationMin = -3.1;
...
CCREATE(Accelerometer, PHIDDEF_ACCELEROMETER)
...
CGET(Accelerometer,AxisCount,int)
	TESTPTRS(phid,pVal) 
	TESTDEVICETYPE(PHIDDEF_ACCELEROMETER)
	TESTATTACHED
...
CSETINDEX(Accelerometer,AccelerationChangeTrigger,double)
	...
	if(phid->phid.networkInfo)
		ADDNETWORKKEYINDEXED(Trigger, "%lf");
	else
