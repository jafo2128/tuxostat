...
CMAKEPACKETINDEXED(TextLCD)
	...
	{
		case PHIDSPEC_TEXTLCD_2_20:
		case PHIDSPEC_TEXTLCD_2_20_CUSTOM:
		case PHIDSPEC_TEXTLCD_2_20_COMP1:
		case PHIDSPEC_TEXTLCD_with_888:
			switch(Index)
			{
...
CCREATE(TextLCD, PHIDDEF_TEXTLCD)

CGET(TextLCD,RowCount,int)
	TESTPTRS(phid,pVal) 
	TESTDEVICETYPE(PHIDDEF_TEXTLCD)
	TESTATTACHED
...
PHIDGET21_API int CCONV CPhidgetTextLCD_setCustomCharacter (CPhidgetTextLCDHandle phid, int Index, int Val1, int Val2)
{
	...
	if(phid->phid.networkInfo)
	{
		...
		ADDNETWORKKEYINDEXED(CustomCharacter, "%s");
	}
	else
	{
		switch(phid->phid.deviceIDSpec)
...
CSETINDEX(TextLCD, DisplayString, char *)
	...
	if (strlen(newVal) > phid->phid.attr.textlcd.numColumns) return EPHIDGET_INVALIDARG;

	if(phid->phid.networkInfo)
		ADDNETWORKKEYINDEXED(DisplayString, "%s");
	else
	{
		switch(phid->phid.deviceIDSpec)
		{
