...
CPHIDGETINIT(MotorControl)
	...
	{
		case PHIDSPEC_MOTORCONTROL:
		case PHIDSPEC_MOTORCONTROL_HC:
					...
					if(error[i])
					{
						...
						if (phid->phid.fptrError)
							if(CPhidget_statusFlagIsSet((CPhidgetHandle)phid, PHIDGET_ATTACHED_FLAG))
								phid->phid.fptrError((CPhidgetHandle)(&phid->phid), phid->phid.fptrErrorptr, EPHIDGET_OUTOFBOUNDS, error_buffer);
					}
				}
...
CCREATE(MotorControl, PHIDDEF_MOTORCONTROL)
...
CGET(MotorControl,MotorCount,int)
	TESTPTRS(phid,pVal) 
	TESTDEVICETYPE(PHIDDEF_MOTORCONTROL)
	TESTATTACHED
...
CSETINDEX(MotorControl,Velocity,double)
	...
	if(phid->phid.networkInfo)
		ADDNETWORKKEYINDEXED(Velocity, "%lf");
	else
