...
int CCONV manager_attach_handler(CPhidgetHandle phid, void *pdss) {
	...
	snprintf(key, MAX_KEY_SIZE, "/PSK/List/%s/%d/%d/%d", 
	phid->deviceType, phid->serialNumber, phid->deviceIDSpec, phid->deviceVersion);
...
//I don't think this one's really neccesary
