...
int zeroconf_advertise_phidget_name(CPhidgetHandle phid, char *mdns_name);
...
static void phidget_group_callback(AvahiEntryGroup *g, AvahiEntryGroupState state, void *userdata) {
    ...
    switch (state) {
        case AVAHI_ENTRY_GROUP_ESTABLISHED :
            ...
            DPRINT("Service '%s' successfully established.", mdns_name);
            break;
        ...
        case AVAHI_ENTRY_GROUP_COLLISION : {
            ...
            DPRINT("Service name collision, renaming service to '%s'", serverName);
            
            /* And recreate the services */
			zeroconf_advertise_phidget_name(phid, n);
            break;
        }

        case AVAHI_ENTRY_GROUP_FAILURE :

            DPRINT("Entry group failure: %s", avahi_strerror_ptr(avahi_client_errno_ptr(avahi_entry_group_get_client_ptr(g))));
...
static void entry_group_callback(AvahiEntryGroup *g, AvahiEntryGroupState state, AVAHI_GCC_UNUSED void *userdata) {
    ...
    switch (state) {
        case AVAHI_ENTRY_GROUP_ESTABLISHED :
            /* The entry group has been established successfully */
            DPRINT("Service '%s' successfully established.", serverName);
            break;
        ...
        case AVAHI_ENTRY_GROUP_COLLISION : {
            ...
            avahi_free_ptr((void *)serverName);
            serverName = n;
	...
	if(Dns_sdInitialized)
	{
		...
		if (!zeroconf_dict_ref)
			if (!(zeroconf_dict_ref = avahi_entry_group_new_ptr(client, entry_group_callback, NULL))) {
				DPRINT("avahi_entry_group_new() failed: %s", avahi_strerror_ptr(avahi_client_errno_ptr(client)));
				goto fail;
			}
				
		snprintf(txtvers, sizeof(txtvers), "txtvers=%s", dnssd_txt_ver);
		...
		{
			DPRINT("Failed to add _phidget_ws._tcp service: %s", avahi_strerror_ptr(ret));
			goto fail;
		}
		/* Tell the server to register the service */
		if ((ret = avahi_entry_group_commit_ptr(zeroconf_dict_ref)) < 0) {
			DPRINT("Failed to commit entry_group: %s\n", avahi_strerror_ptr(ret));
			goto fail;
		}
fail:
...
int zeroconf_advertise_phidget(CPhidgetHandle phid)
{
		...
		CPhidget_getDeviceName(phid, &name);
		snprintf(mdns_name, sizeof(mdns_name), "%s (%d)", name, phid->serialNumber);
		return zeroconf_advertise_phidget_name(phid, mdns_name);
}
...
int zeroconf_advertise_phidget_name(CPhidgetHandle phid, char *mdns_name)
{
	if(Dns_sdInitialized)
	{
		...
		if ((ret = avahi_entry_group_add_service_ptr(phid->dnsServiceRef, AVAHI_IF_UNSPEC, AVAHI_PROTO_UNSPEC, 
			...
			txt_ver, txt_name, txt_type, txt_serial, txt_server_id, txt_label, txt_txtver, txt_protocolver, txt_auth, NULL)) < 0)
		{
			DPRINT("Failed to add _phidget._tcp service: %s", avahi_strerror_ptr(ret));
			goto fail;
		}
		/* Tell the server to register the service */
...
dlsym_good:
    ...
    if (!(threaded_poll = avahi_threaded_poll_new_ptr())) {
        DPRINT("Failed to create threaded poll object.");
	...
	if(!strcmp(serverName, "")) serverName = strdup(avahi_client_get_host_name_ptr(client));

    /* Check wether creating the client object succeeded */
    if (!client) {
        DPRINT("Failed to create client: %s", avahi_strerror_ptr(error));
        return EPHIDGET_UNEXPECTED;
    }
	...
	while(!Dns_sdInitialized)
	{
		...
		if(!timeout)
		{
			...
			DPRINT("InitializeZeroconf Seems bad...");
			return EPHIDGET_UNEXPECTED;
		}
	}
