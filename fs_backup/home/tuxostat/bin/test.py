#!/usr/bin/env python
# Time-stamp: "2008-06-04 13:43:50 tuxostat"
...
def getCRC(s):
    global crcLookupTable

    crcValue = 0x0FFFF # seed the CRC

    # calculate the CRC using the lookup table
    for ch in s:
        crcValue = (crcValue >> 8) ^ crcLookupTable[((crcValue ^ ord(ch)) & 0xFF)]
...
data = ""
dataLen = 0
temp = struct.pack("BB", command, dataLen)
CRC = getCRC(temp)
print "command: "+str(command)
print "data: "+data
print "dataLen: "+str(dataLen)
print "CRC: "+str(CRC)
buffer = struct.pack("BBH", command, dataLen, CRC)
...
# make the struct
command = 0x1F
data = chr(0)+chr(0)+"!!>This is a test<!!"
dataFmt = str(len(data))+"s"
dataLen = len(data)
structFmt = "BB"+dataFmt+"H" # unsigned char command, unsigned char dataLen, char[] data, unsigned short crc
temp = struct.pack("BB"+dataFmt, command, dataLen, data)
...
print "Generated CRC: "+hex(getCRC(temp))
print "CRC: "+hex(CRC)
print buffer
...
# SET LCD BRIGHTNESS to half
print "backlight half"
...
print "backlight full"

# SET LCD BRIGHTNESS to ON
...
"""
    ...
    memcpy(&outgoing_response.data[2],">>>This is a test<<<",20);
